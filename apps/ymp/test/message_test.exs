defmodule YMP.MessageTest do
  use ExUnit.Case

  test "new" do
    message = YMP.Message.new("host1", "protocol1", "service1", "action1",
    ...
    assert message["sender"]["host"] == YMP.get_host()
    assert message["sender"]["protocol"] == "protocol2"
    assert message["sender"]["service"] == "service2"
  end

  test "new_answer" do
    message = YMP.Message.new(YMP.get_host(), "protocol1", "service1", "action1",
                              %{a: 1}, "protocol2", "service2")
    answer = YMP.Message.new_answer(message, %{b: 2})
    assert map_size(message) == 7
    assert answer["id"] |> String.length >= 1
    assert answer["id"] != message["id"]
    assert answer["reply-to"] == message["id"]
    assert answer["host"] == YMP.get_host()
    ...
    assert answer["sender"]["host"] == YMP.get_host()
