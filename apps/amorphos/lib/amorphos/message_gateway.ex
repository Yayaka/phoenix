defmodule Amorphos.MessageGateway do
  if Mix.env == :test do
    @timeout 200
  else
    @timeout 5000
  ...
  @service_protocols Application.get_env(:amorphos, :service_protocols)
  ...
  def push(message) do
    ...
    if to_me do
      ...
      with {:ok, host_information} <- Amorphos.HostInformationProvider.request(host),
           {:ok, connection} = Amorphos.ConnectionProvider.request(host_information) do
        Amorphos.Connection.send_packet(connection, [message])
        :ok
      else
        _ ->
          :error
